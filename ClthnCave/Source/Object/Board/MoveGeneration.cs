...
namespace Chess
{
    public static class MoveGeneration
    {
        ...
        public static void GenerateSlidingMoves(Board board, int sourceSquare, List<Move> pseudoLegalMoves)
        {
            ...
            for (int d = startDirectionIndex; d < endDirectionIndex; d++)
            {
                for (int i = 0; i < numSquaresToEdge[sourceSquare][d]; i++)
                {
                    ...
                    else if(Piece.IsPieceColor(targetPiece, Piece.GetPieceColor(piece)) || Piece.IsPieceType(targetPiece, Piece.block))
                    {
                        break;
                    }
                    else
        ...
        public static void GeneratePawnMoves(Board board, int sourceSquare, List<Move> pseudoLegalMoves)
        {
            ...
            int up = Board.IsPieceInTopSide(board, piece) ? directions[0] : directions[2];

            if((Board.IsPieceInTopSide(board, piece) && numSquaresToEdge[sourceSquare][0] > 0) || (Board.IsPieceInBottomSide(board, piece) && numSquaresToEdge[sourceSquare][2] > 0))
            {
                ...
                if(Piece.IsPieceType(board.pieces[targetSquareUp], Piece.none))
                {
                    ...
                    if((Board.IsPieceInTopSide(board, piece) && numSquaresToEdge[sourceSquare][0] == 1) || (Board.IsPieceInBottomSide(board, piece) && numSquaresToEdge[sourceSquare][2] == 1))
                    {
                ...
                if(numSquaresToEdge[sourceSquare][1] > 0)
                {
                    ...
                    if(!Piece.IsPieceType(board.pieces[targetSquareUpRight], Piece.none) && Piece.CanAttack(piece, board.pieces[targetSquareUpRight]))
                    {
                ...
                if(numSquaresToEdge[sourceSquare][3] > 0)
                {
                    ...
                    if(!Piece.IsPieceType(board.pieces[targetSquareUpLeft], Piece.none) && Piece.CanAttack(piece, board.pieces[targetSquareUpLeft]))
                    {
        ...
        public static void GenerateKnightMoves(Board board, int sourceSquare, List<Move> pseudoLegalMoves)
        {
            ...
            foreach(var square in preComputedKnightMoves[sourceSquare])
            {
                ...
                else if(Piece.CanAttack(piece, board.pieces[square]))
                {
                    pseudoLegalMoves.Add(new Move(sourceSquare, square));
                }
            }
